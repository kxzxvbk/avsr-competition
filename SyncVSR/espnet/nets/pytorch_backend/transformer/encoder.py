...
class Encoder(torch.nn.Module):
    ...
    def __init__(
        ...
        if input_layer == "conv1d":
            ...
            self.resnet = timm.create_model("resnet18")
        else:
            self.frontend = None
        # -- backend module.
        ...
        elif input_layer in ["conv1d", "conv3d", "conv3d-lrw"]:
            self.embed = torch.nn.Sequential(
    ...
    def forward(self, xs, masks, extract_resnet_feats=False):
        ...
        """
        if self.input_layer == "conv3d" :
            if isinstance(self.frontend, (Conv1dResNet, Conv3dResNet)):
                xs = self.frontend(xs)
        elif self.input_layer == "conv3d-lrw":
            xs = torch.transpose(xs, 1, 2)
            xs = self.forward_videos(xs)
